...
public class CipherUtil {
    ...
    public static boolean validatePassword(String password, String inputString) {
        if (password.equals(encodeByMD5(inputString))) return true;
        else return false;
    }

    public static String validatePasswordFormat(String password) {
        if (password.length() < PWD_MIN_LEN) return Property.ERROR_PWD_SHORT;
        if (password.length() > PWD_MAX_LEN) return Property.ERROR_PWD_LONG;
